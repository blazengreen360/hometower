...
|---|---|---|---|
| Project-Manager | Refactoring directive (which files, why) | Restructured code + passing tests | Project-Manager (routes to `CI-Gatekeeper`, then two independent `Code-Reviewer` lanes) |
...
**Automated TDD Re-Generation**: If you successfully extracted domain logic into a new pure functional helper, return that testing follow-up to Project-Manager so PM can route `Test-Automation-Engineer` for atomic `pytests` on the new separated file if needed.
