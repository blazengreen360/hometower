...
Before any simulation, verify the app is reachable:

```bash
curl -sf http://localhost:8080/api/health > /dev/null
```

If the health check fails, **do not proceed**. Return immediately to PM with:
```json
{ "status": "BLOCKED", "blocker_details": "App not reachable at http://localhost:8080 — stack must be running before simulation can proceed. Run: docker compose up -d", "follow_up_required": true }
```

Do not attempt to start the stack yourself. Returning a clear blocked status is the correct behavior.
