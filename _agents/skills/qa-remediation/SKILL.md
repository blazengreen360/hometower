...
`QA-Fixer` is the stable Project-Manager routing alias for this `qa-remediation` skill/agent.
...
|---|---|---|---|
| Project-Manager | Bug report (`doc/bugs/`) or security report (`doc/security/`) | Fixed code + remediation ledger | Project-Manager (routes to `CI-Gatekeeper`, then two independent `Code-Reviewer` lanes) |
...
### PHASE 7: ARCHIVE
When — and only when — the pipeline verdict is `ALL_CLEAR`, a current-pipeline `CI-Gatekeeper` report has passed, and two independent `Code-Reviewer` lanes have approved:
...
- Do **not** archive before a passing current-pipeline `CI-Gatekeeper` report and two independent `Code-Reviewer` approvals
