...
def resolve_changed_files(root: Path, args: argparse.Namespace) -> tuple[list[str], list[str]]:
    ...
    return explicit_files, changed_files


def evaluate_scope(active_role: str | None, reviewer_mode: bool, changed_files: list[str]) -> tuple[bool, list[dict[str, str]], list[dict[str, str]]]:
    if active_role is None:
        return False, [], []
    analysis_only_role = reviewer_mode or active_role == "ci-gatekeeper"
    role_findings = evaluate_role_scope(active_role, changed_files, ignore_read_only=analysis_only_role)
    if analysis_only_role:
        return analysis_only_role, role_findings, []
    return analysis_only_role, [], role_findings


def changed_line_count_for_path(
    path: str,
    explicit_files: list[str],
    untracked_files: set[str],
    changed_line_map: dict[str, int],
    totals: dict[str, int | bool | None],
) -> int:
    changed_lines = changed_line_map.get(path)
    if changed_lines is not None:
        return changed_lines
    if totals["exists"] is not True:
        return 0
    if path in untracked_files or explicit_files:
        return int(totals["total_lines"] or 0)
    return 0


def build_line_counts(
    root: Path, changed_files: list[str], explicit_files: list[str], changed_line_map: dict[str, int]
) -> list[dict[str, int | bool | None | str]]:
        ...
        line_counts.append(
            {
                ...
                "changed_lines": changed_line_count_for_path(
                    path, explicit_files, untracked_files, changed_line_map, totals
                ),
            }
        )
    return line_counts
...
def build_payload(
    ...
    return {
        ...
        "scope_advisories": {
            "mode": "analysis-only" if analysis_only_role else "none",
            "role": active_role,
...
def main() -> int:
    args = parse_args()
    root = repo_root()
    explicit_files, changed_files = resolve_changed_files(root, args)
    active_role = "code-reviewer" if args.reviewer_mode else args.role
    analysis_only_role, advisory_violations, violations = evaluate_scope(
        active_role, args.reviewer_mode, changed_files
    )
    changed_line_map = diff_changed_lines(root, changed_files)
    line_counts = build_line_counts(root, changed_files, explicit_files, changed_line_map)
