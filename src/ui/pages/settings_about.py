...
def _row(label: str, value: str) -> None:
    with ui.row().classes("justify-between w-full").style("gap: 8px;"):
        ui.label(label).classes("ht-small-copy")
        ui.label(value).classes("text-[var(--ht-text-primary)] text-[0.875rem]")


def _section(title: str) -> None:
    ui.separator()
    ui.label(title).classes("ht-section-caption")
    ...
    with app_shell("About", "/settings/about", breadcrumb=["Settings", "About"]):
        with page_container(ui.column()):
            render_page_intro(
                ui,
                "About Hometower",
                "System health, version details, and inventory scale at a glance for quick operational checks.",
                "Settings",
            )

            with card_surface(ui.card()):
                with card_section(ui.column()).classes("w-full"):
