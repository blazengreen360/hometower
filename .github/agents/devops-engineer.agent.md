...
> Execution note: When the main agent delegates this role in a runtime that supports subagents, run it as a bounded `worker` subagent. Return infra changes and the required handshake to the caller, and do not spawn further subagents unless an exemption in `AGENTS.md` explicitly allows it.
...
Do not run `verify-gate`, `pytest`, `mypy`, or architecture greps — those are owned by `CI-Gatekeeper`. Return your infra changes to PM; PM routes the diff to `CI-Gatekeeper` for the formal gate run, then to both `Code-Reviewer` lanes.
