...
description: Deterministic local-runtime tooling for Hometower review enforcement. Use when you need JSON output for the mandatory review gates, or when you need to verify Project-Manager, CI-Gatekeeper, and Code-Reviewer skill docs still contain the required scope, trust-boundary, and approval-policy language.
---

# Deterministic Review Tooling

Use this repo-local skill when review evidence needs to be stable, machine-readable, and local to runtime behavior.
...
.venv/bin/python .github/skills/deterministic-review-tooling/scripts/scope_guard.py --role backend-engineer --file src/services/device_service.py
.venv/bin/python .github/skills/deterministic-review-tooling/scripts/build_review_bundle.py --role ci-gatekeeper
...
- `scripts/scope_guard.py`
  Checks a proposed file list against local role boundaries and protected PM documents. Supports repeated `--file` arguments and `--files-from <path>`.
...
- Keep usage local to repo role skills and repo-local enforcement.
- Do not point this bundle at `.github/`; it exists to harden local runtime behavior.
