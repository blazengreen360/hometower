...
- **Jobs-to-be-Done (Christensen, 2003)**: frame each feature in terms of the user outcome, not the implementation request
- **Kano Model (Kano, 1984)**: distinguish Basics, Performance improvements, and Delighters when prioritizing work
- **MoSCoW**: classify scope as Must, Should, Could, or Won't
- **Minimal Viable Story**: prefer the smallest independently valuable slice

Use these rules when refining stories, resolving tradeoffs, or deciding what should ship next.
...
- Apply **Progressive Disclosure**: default to summary-first, reveal dense controls only when needed
- Apply **Fitts's Law**: primary actions, drag handles, and canvas controls must be large and discoverable
- Apply **Norman's Principles**: interactive elements should show affordance, feedback, constraints, and natural mapping
- Preserve Hometower language: use `Workspace`, `Topology`, and `History`
