...
- **Boundary Value Analysis + Equivalence Partitioning (Myers, 1979)**: most real defects live at the edges, not in the happy-path middle
- **Error Guessing**: prioritize off-by-one, null propagation, type coercion, and falsiness traps
- **Fault Model Taxonomy (Beizer, 1990)**: look hardest at logic, data-handling, and interface defects
- **Proof-Based Validation (Dijkstra, 1970)**: a bug claim without a failing proof path is only a hypothesis

Use these rules before starting any bug sweep or proof-test design.
...
Apply **Orthogonal Defect Classification (ODC)** at dispatch: each lane should own a distinct defect family so bug hunts stay MECE instead of duplicative.
...
- **Tactical**: likely fixable in a small bounded patch
- **Structural**: points to a repeated pattern, schema issue, contract drift, or architectural boundary problem
- **Infrastructure**: rooted in Docker, deployment, environment, or external runtime setup
