...
## Post-Review Remediation Scope (20 Apr 2026)

The canvas bug sweep and UX audit identified additional must-fix interaction defects that are now part of HT-077 closeout acceptance. This section is normative for current implementation.

### Added Must-Fix Defects

1. Container drag must be gated by pre-selection in the same way as leaf-node structural drag.
2. Selection normalization must not run on stale pre-tap state (`pointerdown` race); it must run on settled selection state.
3. Parent/container preview growth during tentative drag must fully rollback on cancel/snap-back.
4. Multi-select drag ownership must be assigned for all selected nodes at grab-start, not only the primary grabbed node.
5. Reparent/detach intent must be visible during drag (`drop target` and `will detach` indicators).
6. Context menu must open at the actual pointer event position for Cytoscape `cxttap` and DOM `contextmenu`.
7. Context menu fallback hit-testing must prefer rendered bounds hit checks over center-point proximity.
8. Context menu dedup between Cytoscape and DOM events must be deterministic (no fragile short timeout race).
9. Mode transition during an active drag must defer drag cleanup until gesture settlement is complete.
10. Drag settlement must avoid parent/child ownership races and never orphan children during parent move.

### Added Acceptance Scenarios

1. Given parent container A is selected and child node B is not selected,
   when the user first click-drags B,
   then B is selected and no container move or structural change is committed on that first pointer cycle.

2. Given a user drag is rejected or cancelled,
   when the node snaps back,
   then parent/container preview geometry and position are restored to pre-drag values.

3. Given multiple selected nodes are dragged,
   when the drag ends,
   then all selected nodes participate consistently in gesture ownership and no non-primary selected node spuriously snaps back.

4. Given a user right-clicks a node or container,
   when the menu opens,
   then the menu anchors to the actual pointer location and targets the hit element under rendered-bounds rules.

5. Given a mode switch occurs while dragging,
   when the gesture settles,
   then cleanup runs once with no stale-origin crashes, no orphaning, and no partial mutations.

Reference artifact: `doc/bugs/canvas-bug.md`.
