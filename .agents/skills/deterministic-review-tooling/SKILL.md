...
description: Deterministic Codex-local tooling for Hometower review enforcement. Use when you need JSON output for the mandatory review gates, or when you need to verify Project-Manager and Code-Reviewer skill docs still contain the required scope, trust-boundary, and approval-policy language.
---
...
- `scripts/run_review_gates.py`
  Runs the mandatory review gates from `AGENTS.md` in fixed order and emits JSON with exact command text, exit code, duration, and captured output hashes.
- `scripts/check_review_policy.py`
  Verifies the local reviewer, PM, and PM handoff skills still contain the required mandatory-gate, trust-boundary, and terminal-scope clauses. Emits JSON pass/fail per file and rule.
...
- During `Code-Reviewer` work, run `scripts/run_review_gates.py` and cite its JSON output in the verdict.
...
- During `Project-Manager` or `pm-handoff` upkeep, run `scripts/check_review_policy.py` after editing reviewer or PM skill prompts.
